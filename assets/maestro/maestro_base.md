...
6. **Integridad de DB**: Queda ESTRICTAMENTE PROHIBIDO ejecutar comandos destructivos de base de datos como `php artisan migrate:fresh` o equivalentes (ej: `npx prisma migrate reset`). Se debe respetar siempre el flujo de migraciones incrementales para asegurar la consistencia en todos los entornos (desarrollo y producción).
